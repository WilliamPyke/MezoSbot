...
BEGIN
  ...
  UPDATE users
  SET balance_sats = balance_sats - reward_sats, updated_at = now()
  WHERE discord_id = q.creator_id AND balance_sats >= reward_sats;
  GET DIAGNOSTICS rows_updated = ROW_COUNT;
  IF rows_updated = 0 THEN
    UPDATE quests SET status = 'exhausted', completed_at = COALESCE(completed_at, now()), updated_at = now()
    WHERE id = p_quest_id;
    RETURN jsonb_build_object('ok', false, 'reason', 'insufficient_creator_balance', 'insertedCompletion', false, 'completedTaskCount', 1, 'tierRewardSats', reward_sats, 'previousPaidSats', COALESCE(previous_paid, 0), 'rewardDeltaSats', reward_sats);
  END IF;

  UPDATE users SET balance_sats = balance_sats + reward_sats, updated_at = now()
  WHERE discord_id = p_user_id;

  INSERT INTO quest_reward_events (quest_id, user_id, reward_delta_sats, total_paid_sats, completed_task_count, reason)
  VALUES (p_quest_id, p_user_id, reward_sats, COALESCE(previous_paid, 0) + reward_sats, 1, 'repeatable_task');

  RETURN jsonb_build_object('ok', true, 'insertedCompletion', false, 'completedTaskCount', 1, 'tierRewardSats', reward_sats, 'previousPaidSats', COALESCE(previous_paid, 0), 'rewardDeltaSats', reward_sats, 'totalPaidSats', COALESCE(previous_paid, 0) + reward_sats);
END;
